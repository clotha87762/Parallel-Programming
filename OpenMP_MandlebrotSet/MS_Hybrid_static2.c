...
int main(int argc,char** argv)
{
 ...
 //   slaveNum = processNum-1;
	...
	if(i<numX%processNum){
			startX += (numX/processNum) + 1;
		}
	else{
			startX +=  (numX/processNum) ;
		}
	}
	...
	store = (int*) malloc(sizeof(int)*numY*widthCount);
		...
		#pragma omp parallel num_threads(threadNum) private(i, j) 
		{
		    ...
		    for(i=0;i<numY; i++) {
			    for(j=startX; j<startX+widthCount; j++) {
			    	Compl z, c;
		  			int repeats;
	...
	if(rank==0){
		for(i=0;i<numX*numY;i++){
			XSetForeground (display, gc,  1024 * 1024 * (pixel[i] % 256));		
			XDrawPoint (display, window, gc, i/numY, i%numY);
