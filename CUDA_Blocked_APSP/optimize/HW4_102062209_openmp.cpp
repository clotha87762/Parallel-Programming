...
//	fprintf(stderr,"tempVertex:%d\n",tempVertex);
	...
	for(int i=0;i<DEVICE_NUM;i++){
		...
		cudaMalloc((void**)&cuda_graph[i],sizeof(int)*(tempVertex)*(tempVertex));
	...
	#pragma omp parallel num_threads(DEVICE_NUM)
	{
			...
			cudaMemcpy( cuda_graph[i],graph,sizeof(int)*tempVertex*tempVertex ,H2D);
	...
	if( bf ==32){	
			...
			for(int K=0;K<FWblockDim;K++){
				...
				#pragma omp parallel num_threads(DEVICE_NUM)
				{
					...
					floyd_warshall_1<<<1,threadStr>>>(cuda_graph[threadId],K,K*bf);
					...
					if(FWblockDim>1){
							...
							floyd_warshall_2<<< ((FWblockDim-1))*2 ,threadStr>>>(cuda_graph[threadId],K,K*bf);
							...
							if(threadId==(DEVICE_NUM-1)&&(((FWblockDim-1)%DEVICE_NUM)!=0)){
								...
								floyd_warshall_3<<<Str_last,threadStr>>>(cuda_graph[threadId],K,K*bf,threadId);
								cudaCheckErrors("phase 3 last");	
							}
							else if((FWblockDim-1)/DEVICE_NUM!=0){
								floyd_warshall_3<<<Str_normal,threadStr>>>(cuda_graph[threadId],K,K*bf,threadId);
				...
				if(FWblockDim>1){
					...
					#pragma omp parallel num_threads(DEVICE_NUM)
					{
							...
							for(int j=0;j<DEVICE_NUM;j++){
								...
								cudaMemcpy(cuda_graph[i]+offset,cuda_graph[j]+offset,count,D2D);
			...
			for(int K=0;K<FWblockDim;K++){
				...
				{	
					...
					//printf("K=%d phase1\n",K);
					for(int i=0;i<bf;i++){
						floyd_warshall_beta_1<<<bf,bf>>>(cuda_graph[threadId],K,K*bf + i);
						cudaCheckErrors("phase 1");
					}
					...
					if(FWblockDim>1){
						for(int i=0;i<bf;i++){
							floyd_warshall_beta_2<<<(FWblockDim-1)*2*bf,bf>>>(cuda_graph[threadId],K,K*bf + i );
							cudaCheckErrors("phase 2 col");
						}
						...
						if(threadId==(DEVICE_NUM-1)&&(((FWblockDim-1)%DEVICE_NUM)!=0)){
							floyd_warshall_beta_3<<<Str_last,bf>>>(cuda_graph[threadId],K,K*bf,FWblockDim-1,threadId);
						}
						else if((FWblockDim-1)/DEVICE_NUM!=0){
							floyd_warshall_beta_3<<<Str_normal,bf>>>(cuda_graph[threadId],K,K*bf,FWblockDim-1,threadId);
						}
	...
	cudaMemcpy(graph,(cuda_graph[0]),sizeof(int)*tempVertex*tempVertex,D2H);
