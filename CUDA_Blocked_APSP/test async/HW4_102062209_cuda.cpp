...
int main(int argc,char* argv[]){
	...
	if( bf ==32 && edge_num/total_vertex <= 6){	
		for(int K=0;K<FWblockDim;K++){
				printf("K=%d phase1\n",K);
			// Phase 1
		
				floyd_warshall_2<<< (FWblockDim-1)*2 ,threadStr 0, stream[0]>>>( cuda_graph,K,K*bf);
				Errors("phase 1");
			...
			//printf("K=%d phase2\n",K);
				...
				floyd_warshall_2<<< (FWblockDim-1)*2 ,threadStr , 0 , stream[1]>>>( cuda_graph,K,K*bf);
				...
				floyd_warshall_3<<<blockStr,threadStr,0, stream[2]>>>(cuda_graph,K,K*bf);
